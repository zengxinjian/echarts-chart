...
this.onmessage = function(event) {
	var ip = "ws://" + location.hostname + ":7880"; //169.254.10.225 169.254.10.66
	var ip2 = "ws://169.254.10.225:7880";
	var ip3 = "ws://127.0.0.1:4649/Echo?name=pengbo";
	var ip4 = "ws://192.168.1.208:4649/Echo?name=pengbo";
	var that = this;
	var pcmArray = [];
	var signal = [];
	...
	//var real=new Float64Array(16384);
	//var imag=new Float64Array(16384);
	pcmsocket.onopen = function(ev) {
		//连接的时候发送用户的id的
		pcmsocket.send(parseInt(Math.random() * 100));
	}
	pcmsocket.onclose = function(ev) {
		//断开连接后进行重新连接
		onmessage(ev);
	}

	pcmsocket.onmessage = function(ev) {
		//console.log("onmessage--");
		//console.log(ev);
		var arr = [];
		arr = ev.data.split(',');
		that.postMessage(arr);
		//console.log(arr);
		if (sendCount++ > 10) {
			pcmsocket.send("active");
			sendCount = 0;
		}
		/* if (arr[0] === "Q") {
			console.log("onmessage-Q----")

			//real = arr.splice(2, 16386);

			var frameBufferSize = 32768;
			var bufferSize = frameBufferSize / 2;
			var signal = new Float32Array(bufferSize);
			var peak = new Float32Array(bufferSize);
			var fft = new FFT(bufferSize, 44100);
			arr.shift(); //把数组的第一个元素从其中删除
			arr.shift();
			console.log("arr.length--", arr.length);
			console.log("arr--", arr);
			//FFT计算前进行数据处理
			//in[i][1] = (((qreal)(image[i]))/32768)/2.5*(0.5-0.5*cos(2*M_PI*i/fftPoint))*1.633;//image   
			for (var i = 0; i < arr.length; i++) {
				console.log("arr-0-", arr[i]);
				arr[i] = (((arr[i])) / 32768) / 2.5 * (0.5 - 0.5 * Math.cos(2 * Math.PI * i / arr.length)) * 1.633
				console.log("arr-1-", arr[i]);
			}

			fft.forward(arr);
			console.log("fft.spectrum--", fft.spectrum);
			that.postMessage(fft.spectrum);
		} else {
			console.log("onmessage-else--I-")
			that.postMessage(arr);
			arr.shift();
			arr.shift();
			//FFT计算前进行数据处理
			//in[i][1] = (((qreal)(image[i]))/32768)/2.5*(0.5-0.5*cos(2*M_PI*i/fftPoint))*1.633;//image   
			img = arr.splice(2, 8192);
			for (var i = 0; i < img.length; i++) {
				//console.log("img-0-", img[i]);
				img[i] = (((img[i])) / 32768) / 2.5 * (0.5 - 0.5 * Math.cos(2 * Math.PI * i / img.length)) * 1.633
				//console.log("img-1-", img[i]);
			}
			console.log("img--", img);
			var frameBufferSize = 32768;
			var bufferSize = frameBufferSize / 2;
			...
			fft.forward(img);
			that.postMessage(fft.spectrum);
			//img = arr.splice(2, 16386) //删除函数：位置，数量

		} */
		/* console.log("real-Q----",real.length)
		console.log("img-I----",img.length)
		if (img.length > 0 && real.length > 0) {
			console.log("fftasm.fft----",img.length,real.length);
			fftasm.fft(real, img, true);
			that.postMessage(fftasm.bitrev)
			console.log(fftasm);
			img = [];
			real = [];
		} */
...
FFT.prototype.forward = function(buffer) {
	...
	var bufferSize = this.bufferSize,
		...
		reverseTable = this.reverseTable, //反向
		real = this.real,
		imag = this.imag, //
		spectrum = this.spectrum; //频谱
