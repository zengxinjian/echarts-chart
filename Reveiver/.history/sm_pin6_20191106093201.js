...
var sm_pin = (function() {
    ...
    function updateRubberband(loc) {
		...
		that.canvas.addEventListener('contextmenu', function(ev) {
			...
			clickRightMenu.style.left = event.layerX-80 + "px";
			clickRightMenu.style.top = event.layerY-100 + "px";
			ev.preventDefault();
			return false;
		});
		//点击重置
    ...
    that.canvas.onmousedown = function (e) {
		...
		that.canvas.onmouseup = function (e) {
			...
			if((x>0 && x<800)&&(y>0 && y<640) && dragging){ // (x - mousedown.startX) > 50 限制最小矩形
				...
				updateRubberband(loc);
				dragging = false;
				if(mousedown.startX !== mousedown.endX) drawlLine(); // 是否是点击立马松开
			}
		};
	};
