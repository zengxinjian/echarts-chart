var sm_spectrogram=(function(){
    ...
    _.prototype.drawHTick=function(){
        ...
        if(this.axis ==="Horizontal"){
            ...
            this.ctx.fillText("单位:Hz", this.canvas.width * 0.1 + 8, this.canvas.height * 0.9 + 16)
        }else{
            for (var i = 0; i < this.canvas.width * 0.8 + 1; i++) {
                ...
                if (i % parseInt(this.canvas.width * 0.8 / 10) === 0) {
                    var vtxt = parseInt(_start_f + i * _length_f / (this.canvas.width*0.8));
                    this.ctx.fillText(vtxt,
            ...
            this.ctx.fillText("单位:Hz", this.canvas.width * 0.8 + 105, this.canvas.height * 0.9 + 28)
        }
		this.ctx.stroke()
    }
    _.prototype.drawVTick=function(){
        var oldHeight = this.canvas.height * 0.8;
        var oldWidth = this.canvas.width * 0.8;
        
        if(this.axis ==="Horizontal"){
            for (var i = 0; i <= parseInt(oldWidth); i++) {
                this.ctx.beginPath();
                this.ctx.strokeStyle = "#ddd";
                this.ctx.font = "14px Arial";
                if (i % parseInt(oldWidth / 5) === 0 && i !== 0) {
                    if(this.drawIndex > parseInt(oldWidth)){
                        var a = this.drawIndex - parseInt(oldWidth) + i;
                        this.ctx.fillText(parseInt(a),
                            i + this.canvas.width * 0.1 + 5,
                            this.canvas.height * 0.9 + 15)
                    }else{
                        this.ctx.fillText(parseInt(i), i + this.canvas.width * 0.1 + 5, this.canvas.height * 0.9 + 15)
                    }
                }
            }
            this.ctx.fillText("单位:s", this.canvas.width * 0.8 + 105, this.canvas.height * 0.9 + 28)
        }else{
            for (var i = 0; i <= parseInt(oldHeight); i++) {
                ...
                if (i % parseInt(oldHeight / 10) === 0 && i !== 0) {
                    if(this.drawIndex > parseInt(oldHeight)){
                        ...
                        this.ctx.fillText(parseInt(a), this.canvas.width * 0.1 - 5,
                            this.canvas.height * 0.1 + i+ (i===parseInt(oldHeight)?0:5));
                    }else{
                        this.ctx.fillText(parseInt(i), this.canvas.width * 0.1 - 5,
                            this.canvas.height * 0.1 + i+ (i===parseInt(oldHeight)?0:5))
                    }
                }
            }
            this.ctx.fillText("单位:s", this.canvas.width * 0.1 - 5, this.canvas.height * 0.1 + 9)
        }
        ...
        if (this.axis ==="Horizontal") {
            if(drawData.length > (this.canvas.width * 0.8)){
                ...
                if (this.drawIndex > parseInt(this.canvas.width * 0.8)) {
                    ...
                    var img = this.ctx.getImageData(parseInt(this.canvas.height * 0.1), parseInt(this.canvas.width*0.1), parseInt(this.canvas.width * 0.8), parseInt(this.canvas.height * 0.8));
                    console.log(parseInt(this.canvas.height * 0.1), parseInt(this.canvas.width*0.1),"语谱")
                    this.ctx.putImageData(img, parseInt(this.canvas.height * 0.1)-1, parseInt(this.canvas.width*0.1));
                    this.allData.shift();
					// 清除Y坐标数值，重新绘制数据
					// this.ctx.clearRect(50,50, this.canvas.width*0.9, 100);
					// this.drawVTick();
                }
                this.drawIndex++;
            }
            ...
            if(drawData.length > (this.canvas.width * 0.8)){
                ...
                if (this.drawIndex > parseInt(this.canvas.height * 0.8)) {
                    ...
                    console.log(parseInt(this.canvas.width * 0.1), parseInt(this.canvas.height*0.1),"瀑布")
                    this.allData.shift();
					// 清除Y坐标数值，重新绘制数据
