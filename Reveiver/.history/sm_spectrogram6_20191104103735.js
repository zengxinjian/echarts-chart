		...
		for (var i = 0; i < this.canvas.height * 0.8 + 1; i++) {
			...
			if (i % parseInt(this.canvas.height * 0.8 / 10) === 0) {
				...
				this.ctx.fillText(vtxt,
					i + this.canvas.height * 0.1 + 5,
					this.canvas.width * 0.9 + 15)
				this.ctx.stroke() //图形绘制
			}

		}
		this.ctx.fillText("单位:Hz",
			this.canvas.height * 0.8 + 105,
			this.canvas.width * 0.9 + 28)
		this.ctx.stroke()
    }
    _.prototype.drawVTick=function(){
        var oldWidth = this.canvas.height * 0.8;
			...
			if (i % parseInt(oldWidth / 10) === 0 && i !== 0) {
				if(this.drawIndex > parseInt(oldWidth)){
					...
					this.ctx.fillText(parseInt(a),
						this.canvas.height * 0.1 - 5,
						this.canvas.width * 0.1 + i+ (i===parseInt(oldWidth)?0:5));
				}else{
